...
```bash
bash scripts/fetch-ticket.sh 163953                    # 스레드 가져오기
bash scripts/fetch-ticket.sh 163953 --images /tmp/t    # 이미지가 있다고 나오면
```

이미지는 그냥 첨부가 아니다. 설정값·에러·경고 배너가 **스크린샷 안에만** 있고 본문
텍스트에는 없는 경우가 많다. 있다고 나오면 받아서 본다.

가져온 내용은 `templates/ticket-intake.md` 를 복사해 `tickets/<번호>.md` 로 기록하고,
분류에 따라 세 산출물 중 하나를 만든다. 형식은 `playbooks/ticket-outputs.md`.
