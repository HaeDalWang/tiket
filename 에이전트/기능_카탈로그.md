...
|---|---|---|---|---|---|---|
| `customer-aws-readonly` | Inspect or troubleshoot customer AWS resources | `aws-customer-account-ops` | Install the same package | Install the same package | Install the same package | Approved broker path only; no write API |
| `fitcloud-billing` | Customer cost, invoice, or savings analysis | `aws-customer-account-ops` | Same package | Same package | Same package | Customer-facing figures must be FitCloud-curated |
...
| `aws-official-research` | AWS specification or support boundary | Prefer AWS MCP documentation search | Available AWS MCP or official web docs | Available MCP/web | Available MCP/web | Separate documentation lookup from customer-account execution |
| `current-web-research` | Release, issue, CVE, or current information | Exa MCP | `web_search`/`web_extract` or Exa MCP | Available web/MCP | Available web/MCP | Cite only content fetched in the current task |
