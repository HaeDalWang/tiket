...
- `플레이북/함정/` is shared curated knowledge whose entries act as routing warnings and always require ticket-specific re-verification; they are not standalone proof.
...
에이전트/환경/mcp-manifest.yaml  secret-free capability manifest
...
- `에이전트/작업_라우터.md` classifies the task and controls selective loading.
- `플레이북/근거_검증.md` implements the portable `answer-quality-gate` capability.
